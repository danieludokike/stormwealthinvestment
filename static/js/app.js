...
jQuery(function ($) {
  ...
  var displays = document.querySelectorAll('.circular-progress');
  var transitionDuration = 900;
  displays.forEach(function (display) {
    var note = Math.round(display.dataset.note);

    var _display$dataset$note = display.dataset.note.split('.'),
        _display$dataset$note2 = _slicedToArray(_display$dataset$note, 1),
        _int = _display$dataset$note2[0];

    var _ref = [Number(_int)];
    _int = _ref[0];
    strokeTransition(display, note);
    increaseNumber(display, _int, 'int');
  });

  function strokeTransition(display, note) {
    var progress = display.querySelector('.circle-progress-fill');
    var radius = progress.r.baseVal.value;
    var circumference = 2 * Math.PI * radius;
    var offset = circumference * (100 - note) / 100;
  ...
  function increaseNumber(display, number, className) {
    var element = display.querySelector(".percent-".concat(className)),
...
__webpack_require__(/*! ../assets/js/owl.carousel.min */ "./resources/assets/js/owl.carousel.min.js");

__webpack_require__(/*! ../assets/js/script */ "./resources/assets/js/script.js");

__webpack_require__(/*! ./telephone */ "./resources/js/telephone.js");

__webpack_require__(/*! ./country-currency */ "./resources/js/country-currency.js");
...
$('input[type=password] + .icon').click(function () {
  $(this).toggleClass('flaticon-visibility flaticon-invisible');
...
$('[type=radio]').on('change', function () {
  if ($(this).check()) {
    $(this).parent().toggleClass('bg-dark');
  }
});
...
if (localStorage.getItem('tab') !== null) {
  ...
  var btn = '#' + $(id).attr('aria-labelledby');
  $(btn).addClass('active');
...
$(document).ready(function () {
  ...
  var intv = setInterval(function () {
    current_progress += Math.floor(Math.random() * 10) + 5;
    if (progress > 10) text = "Validating transaction...";
    if (progress > 50) text = "Transfering data...";
    if (progress > 70) text = "Authorizing...";

    if (progress >= 100) {
      progress = 100;
      clearInterval(intv);
      $('#loader-page').removeClass('d-none');
      $('#cruz-bar').parent().hide();
    }

    $('#cruz-bar').css('width', progress + "%").attr('aria-valuenow', progress).text(text + ' ' + progress + "%");
    progress += current_progress;
  }, 4000);
}); // require('country-select-js/build/js/countrySelect.min')
// $("#country").countrySelect()

/***/ }),

/***/ "./resources/js/country-currency.js":
/*!******************************************!*\
  !*** ./resources/js/country-currency.js ***!
  \******************************************/
/***/ ((__unused_webpack_module, __unused_webpack_exports, __webpack_require__) => {

var cc = __webpack_require__(/*! iso-country-currency */ "./node_modules/iso-country-currency/index.js");

var inputHTML = cc.getAllISOCodes().sort(function (a, b) {
  return a.countryName.localeCompare(b.countryName);
}).map(function (data) {
  return "<option value=\"".concat(data.countryName, "\" ").concat(data.countryName === 'United States' ? 'selected' : '', ">").concat(data.countryName, "</option>");
});
var countrySelectElement = document.querySelector('#select-countries');

if (countrySelectElement) {
  countrySelectElement.innerHTML = inputHTML.join('\n');
  countrySelectElement.addEventListener('change', function (e) {
    document.querySelector(countrySelectElement.getAttribute('data-symbol-target')).value = cc.getAllISOCodes().find(function (d) {
      return d.countryName == e.target.value;
    }).currency;
  });
}
